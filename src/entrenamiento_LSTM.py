...
class TrafficLightPredictor:
    ...
    def save_model(self, filepath=None):
        """
        Guarda el modelo entrenado y los escaladores
        """
        if self.model is None:
    ...
    def load_model(self, filepath=None):
        """
        Carga un modelo previamente guardado y sus scalers
        """
        if filepath is None:
            filepath = self.model_path
...
def export_model_for_production(predictor, filepath='models/production_model.h5'):
    """
    Exporta el modelo con todos sus componentes para su uso en producción
    
    Args:
        predictor: Instancia de TrafficLightPredictor entrenada
...
def run_traffic_light_workflow(data=None, sequence_length=8, epochs=30, batch_size=16):
    ...
    # Exportar modelo para producción también con timestamp
    production_model_path = export_model_for_production(predictor, f'models/production_model_{timestamp}.h5')
    
    # Crear también una copia con nombre fijo para compatibilidad
    latest_path = 'models/latest_production_model.h5'
    if os.path.exists(latest_path):
        os.remove(latest_path)  # Eliminar versión anterior
    shutil.copy(production_model_path, latest_path)
    
    # Copiar también los archivos asociados
    ...
    print("| Este-Oeste | Recto              | 13                   | 31.2s          | rojo    |")
